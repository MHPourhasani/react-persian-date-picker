...
const DayPicker: React.FC<DayPickerProps> = ({
	...
	const getClassName = (object: any) => {
		...
		if ((minDate && date < minDate) || (maxDate && date > maxDate) || object.disabled) {
			names.push(`text-secondary300 cursor-text hover:border-none ${disabledDayClassName}`);
		...
		if (!object.disabled || !onlyShowInRangeDates) {
			if (isSameDate(date, today)) names.push(`border-1.5 border-primary rounded-xl ${todayClassName}`);

			if (isSelected(date)) {
				names.push(`bg-primary text-white ${selectedDayClassName}`);
			}
		}
	...
	return (
		<section className="mt-7 flex w-full flex-col items-center justify-center">
			...
			{months.map((weeks, monthIndex) => (
				<div key={monthIndex} className="mt-2 flex flex-col items-center justify-center">
