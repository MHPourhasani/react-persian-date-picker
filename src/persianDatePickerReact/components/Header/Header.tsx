...
const Header: React.FC<HeaderProps> = ({
	...
	const { date, minDate, maxDate, year } = state,
		isPreviousDisable = minDate && date.year <= minDate.year && minDate.monthIndex > date.monthIndex - 1,
		isNextDisable = maxDate && date.year >= maxDate.year && maxDate.monthIndex < date.monthIndex + 1;
	...
	return (
		<div className="flex w-full items-center justify-between">
			...
			{months.map((month: string, index: number) => (
				...
				>
					{showMonthPicker && !showYearPicker && !showDayPicker && (
						<span
	...
	function getButton(direction: string) {
		const handleClick = () => increaseValue(direction === "right" ? ArrowDirection.Right : ArrowDirection.Left),
			disabled = (direction === "left" && isPreviousDisable) || (direction === "right" && isNextDisable);
