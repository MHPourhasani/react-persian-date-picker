...
const Calendar: React.FC<CalendarProps> = ({
	...
	const [state, setState] = useState<any>({}),
		[showDayPicker, setShowDayPicker] = useState<boolean>(true),
	...
	const getDateInRangeOfMinAndMaxDate = (date: any, minDate: any, maxDate: any, calendar: CalendarType) => {
		if (minDate)
	...
	useEffect(() => {
		...
		setState((state: StateType) => {
			...
			const [selectedDate, $minDate, $maxDate] = getDateInRangeOfMinAndMaxDate(getSelectedDate(value), minDate, maxDate, calendar);
		...
		state.today && (
			...
			>
				...
				<div className="relative flex w-full flex-col items-center justify-center">
					...
					{showYearPicker && <YearPicker {...globalProps} yearState={yearState} />}
					<div className="absolute top-0">
						{showTimePicker && <TimePicker state={state} onChange={handleChange} setShowTimePicker={setShowTimePicker} />}
					</div>
				</div>

				{timePicker && (
					<ClockIcon onClick={() => setShowTimePicker(!showTimePicker)} className="mt-2.5 cursor-pointer !fill-primary" />
				)}
			</section>
		)
	);
