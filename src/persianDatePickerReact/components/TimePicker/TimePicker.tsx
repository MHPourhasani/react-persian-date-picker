...
const TimePicker: React.FC<TimePickerProps> = ({
  ...
  const Button = ({
    ...
    return (
      <div className="flex w-full flex-col items-center gap-20">
        <Arrow
          direction="up"
          onClick={() => updateInput(name, number + 1)}
          disabled={false}
        />
        ...
        <input
          ...
          name={name}
          value={localeValue}
          className="w-7 text-center text-xl focus:outline-none"
        />
  ...
  return (
    <section
      dir="ltr"
      className="flex w-full flex-col items-center rounded-t-3xl bg-white pb-24 pt-8"
    >
      <CalendarIcon
        className="mb-10 cursor-pointer fill-primary"
        onClick={() => setShowTimePicker((prevState: boolean) => !prevState)}
      />
